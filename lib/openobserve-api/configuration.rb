...
module OpenObserve::Api
  class Configuration
    attr_accessor :base_url, :timeout, :logger, :debugging, :query_array_encoding, :api_key, :username, :password
    ...
    def apply_auth(headers, query, auth_names)
      headers['Authorization'] = api_key if api_key && auth_names.include?('Authorization')
      headers['Authorization'] = Faraday::Utils.basic_header_from(username, password) if username && auth_names.include?('BasicAuth')
      nil
    end
